...
class AiService {
  ...
  Future<Message> sendMessage(String text, {String? languageCode}) async {
    ...
    try {
      ...
      final fullPrompt =
          'FECHA ACTUAL: ${DateTime.now().toIso8601String()}\n'
          '$languageInstruction\n'
          '$contextValues\n\n'
          'Usuario: $text';
      ...
      if (jsonMatch != null) {
        try {
          ...
          responseText = responseText
              .replaceRange(jsonMatch.start, jsonMatch.end, '')
              .trim();
        } catch (e) {
          ...
          genUiData = {
            'type': 'parsing_error',
            'raw_content':
                responseText, // Optional: for debug if needed, but not shown to user
            'message':
                'La IA generó una respuesta que no pude entender. Por favor intenta de nuevo.',
          };
